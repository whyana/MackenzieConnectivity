...
write_rds(channels.lakes, "joinedData_20220527.Rdata")
...
```{r}
setwd("C:/Users/whyana/OneDrive - University of North Carolina at Chapel Hill/DocumentsLaptop/001_ Graduate School/Research/Connectivity/Mackenzie/Data/intermediaryDownloads")
channels.lakes=read_rds("joinedData_20220527.Rdata")
...
setwd("C:/Users/whyana/OneDrive - University of North Carolina at Chapel Hill/DocumentsLaptop/001_ Graduate School/Research/Connectivity/Mackenzie/Data/intermediaryDownloads")
write_rds(final.pred.rf, "predictions_traintest_31052022.Rdata")
...
prep=cbind(predict(fitted.wf.rf, geci.val), geci.val) %>% as_tibble() %>% select(OBJECTID, .pred_class, classes)%>% rename(.obs_class = classes) %>% 
  mutate(split=case_when(
    OBJECTID %in% train.ids~"train", 
    OBJECTID %in% test.ids~"test"),
    .pred_class=as.numeric(.pred_class),
    .obs_class = as.numeric(.obs_class)) %>% 
#  dplyr::group_by(split, .pred_class, .obs_class) %>% summarise(count=n()) %>% ungroup() %>% filter(split=="train") %>% 
  select(-split, -OBJECTID) %>% 
  make_long(.obs_class,.pred_class)
