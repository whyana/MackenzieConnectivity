...
# Set folder for all input data
int.wd = "~/WRR Submission 2 Data/Script 2"

#Name of file for lake shapefiles & island polygon shapefiles & river reach shapefiles
lakes.shapeFile = "mackenzieGoodLakes.shp"
import.sword = "na_sword_reaches_hb82_v14.shp"
setwd(int.wd)
lakes.sf = st_read(lakes.shapeFile)
# Piliouras delta islands shapefile
...
setwd(images.wd)
ggsave(paste0(todayDate, "_MonthlyUncertMap.png"), height=3, width=8, units="in", bg="white")
```
...
all.df = all.df %>% 
  mutate(timegroup2= case_when(
    timegroup==0 ~ "0 - 27",
    timegroup==28 ~ "28 - 55",
    timegroup==56 ~"56 - 83",
    timegroup==84 ~ "84 - 111",
    timegroup==112 ~ "112 - 139"
  )) 
all.df$timegroup2 = factor(all.df$timegroup2, levels = c("0 - 27", "28 - 55", "56 - 83", "84 - 111", "112 - 139"))

all.df%>% 
  ggplot(aes(x=thresh, y=pct.area, color = as.factor(timegroup2),
             group=timegroup2))+geom_line()+geom_point()+
  theme_bw()+ylab("probability of\nexceeding connectivity threshold")+
  ...
  scale_color_manual(values=c("#130112", "#A21675", "#E74BB5", "#F093D3", "#96788D"))+
  labs(color="Days after\npeak discharge")

setwd(images.wd)
ggsave(paste0(todayDate, "probabilityOfExceed.pdf"), width=6, height=4, units = "in")
# plot variability of flow - NOT IN PAPER
