...
```{r}
# import lake & channel reflectance
...
all.lakes = fread('MackenzieLakeExport_20230323_2.csv')  %>% select(!matches("p10") & !matches("p90"))
all.channels = fread('MackenzieChannelExport_20230313_allYears.csv')  %>% select(!matches("p10") & !matches("p90"))
...
#lakes.filter = lakes.filter[!(Landsat==7 & month <=6 & year>=2003),]
# remove potentially ice impacted LS 7 lakes during scanline faliure
 

# Filter channel data
channels.maxPix = all.channels %>% group_by(fid) %>% summarise(max.pix = max(Red_count))# count max number of pix--> likely the clear sky total of the lake
...
#channels.filter = channels.filter[!(Landsat==7 & month <=6 & year>=2003),]



# import image metadata & prep
...
# # join reflectance and metadata together
# lakesChan.all = rbind.data.frame(
#   lakes.filter %>% as_tibble(),
#   channels.filter %>% as_tibble() %>% 
#     select(all_of(colnames(lakes.filter %>% as_tibble())))
# )
# 
...
# Define functions to compare all observations to landsat 7
lm5 <- function(band){
  y <- data.join %>% 
...
# Do the same thing but for the channel data
...
# apply functions and correlation
...
# Do the same thing for channels
