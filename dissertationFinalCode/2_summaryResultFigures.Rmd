...
#summarize number of connected lakes
```{r}
# Get yearly timing of peak flow at inuvik
...
connected.area.prep = filt.obs %>% 
  select(.pred_class, OBJECTID, doy, month, year) %>% 
  mutate(.pred_class=as.numeric(as.character(.pred_class))) %>% 
  group_by(OBJECTID) %>% 
  summarise(mean.score=mean(.pred_class, na.rm=T),
            med.score = median(.pred_class, na.rm=T),
...
# do the same thing, but for 2 week groups after peak flow
connected.area.prep = filt.obs %>% 
...
for (z in 1:length(thresh)){
  ...
  for (d in unique(connected.area.prep$timegroup)){
    ...
    combo.df = data.frame(thresh = thrshhld,con.area.km = con.area, pct.area = pct.area, con.vol.pct=con.vol.pct, con.vol=con.vol, timegroup = d)
...
all.df
all.df %>% 
  ggplot(aes(x=thresh, y=pct.area, color = as.factor(timegroup), group=timegroup))+geom_line()+geom_point()+
  theme_bw()+ylab("percent of total lake area")+
  xlab("mean connectivity class threshold")+
  theme(axis.text = element_text(size=12),
        axis.title = element_text(size=12, face="bold"))+
  scale_x_continuous(breaks=c(0,0.5, 1,1.5, 2))+labs(color="days after peak flow")

# ylab label expression(bold(lake~area~greater~than~threshold~'('~km^2~')'))
con.plot=connected.area.prep %>% filter(mean.score>=1) %>% 
  ggplot()+
  geom_sf(data=mack.basin.large, color="grey40", size=0.25)+
  geom_sf(data=connected.area.prep, fill=NA, color=NA)+
  geom_sf(fill="forestgreen", color=NA)+theme_void()

all.plot=connected.area.prep %>% 
  ggplot()+
  geom_sf(data=mack.basin.large, color="grey40", size=0.25)+
  geom_sf(fill="darkorange2", color=NA)+theme_void()
  
all.plot+con.plot
setwd(images.wd)
ggsave("connectedLakesMap.png", width=8, height = 6, units="in")
# save image
setwd(images.wd)
ggsave(paste0(todayDate, "connectedLakeArea.pdf"), width=4, height=4, units = "in")
# compare to piliouras connected lake area 637.1 km2
