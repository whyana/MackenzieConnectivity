...
val.wd = "C:/Users/whyana/OneDrive - University of North Carolina at Chapel Hill/DocumentsLaptop/001_ Graduate School/Research/Connectivity/Mackenzie/Data/MarshHey1988"
...
p1=ggplot()+
  geom_sf(data=sill.sf %>% filter(fin.range<1 & !is.na(fin.sill)),
...
p2.all = ggplot()+
  geom_sf(data=sill.sf %>% filter(!is.na(fin.sill)),
          aes(fill=fin.range), color=NA)+
  scale_fill_viridis_c(option="viridis", limits=c(0,6))+
  geom_sf(data=mack.basin.large, color="grey70", size=0.5)+
  #geom_sf(data=station.locations.sill, color="black",size=2, shape=17)+
  theme_void()+
  theme(
...
p3.all=ggplot()+
  geom_sf(data=sill.sf %>% filter(is.na(fin.sill)) %>% 
            mutate(class = ifelse(class=="discharge dependant", 
                                  "high uncertainty, can't calculate sill", 
                                  class)) %>% 
            mutate(class = ifelse(class=="no discharge relationship", 
                                  "no significant\nwater level relationship", class)),
          aes(fill=class), color=NA)+
  #scale_fill_viridis_c(option="viridis", limits = c(0,1))+
  geom_sf(data=mack.basin.large, color="grey70", size=0.5)+
 # geom_sf(data=station.locations.sill, color="black",size=2, shape=17)+
...
p1+p2+p3
setwd(images.wd)
ggsave(paste0(todayDate,"_sills.png"), width=10.5, height=8.5, units = "in")

p1.all+p2.all+p3.all
setwd(images.wd)
ggsave(paste0("20230423","_sills_ALL.png"), width=10.5, height=8.5, units = "in")
...
combo.df = val.raw %>% st_join(lakes.prj %>% select(geometry, OBJECTID)) %>% 
  ...
  mutate(marsh.sill = (SmmrSill+SpringSill)/2)

combo.df %>% filter(fin.range<=1)%>%  ggplot(aes(x=fin.sill, y=marsh.sill))+
  geom_errorbarh(aes(xmin = fin.sill-errorbar, xmax = fin.sill+errorbar, y=marsh.sill, height=0.1), alpha=0.5, color="grey50")+
  geom_errorbar(aes(x=fin.sill, ymin=marsh.sill-0.5, ymax =marsh.sill +0.5, width=0.1), alpha=0.5, color="grey50")+
  theme_bw()+
  geom_abline(aes(slope=1, intercept=0))+geom_point()+
  coord_fixed(ratio = 1, xlim = NULL, ylim = NULL, expand = TRUE, clip = "on")+
  xlab("functional sill elev. (m)")+
  ylab("Marsh & Hey 1988 Avg Spring & Summer Sill Elevation (m)")+xlim(0,5)+ylim(0,5)


count.df = combo.df %>%  filter(is.na(fin.sill)) %>% group_by(class) %>% count()

combo.df %>%  filter(is.na(fin.sill)) %>% ggplot(aes(x=class, y=marsh.sill, fill=class))+geom_boxplot()+
  geom_text(data=count.df, aes(x=class, y=5.2, label=paste0("n=",n)))+theme_bw()+xlab("lakes w/ no functional sill elevation")+
  ylab("Marsh & Hey 1988 Avg Spring & Summer Sill (m)")
...
combo.df %>% mutate(sill.diff = marsh.sill-fin.sill) %>% filter(!is.na(fin.sill)) %>% 
