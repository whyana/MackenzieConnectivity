...
int.wd="~/WRR Submission 2 Data/Script 3"
...
import.sword = "na_sword_reaches_hb82_v14.shp" # River centerlines


# Working directory for image exports
images.wd = "~/images"

# import Marsh & Hey Validation
val.filename = "MarshHey1998_prj.shp"

# River waterlevel data working directory
wsc.wd = "~/WRR Submission 2 Data/Script 3/waterLevelData"
...
# Export the intermediary step to save time for later code runs
setwd(int.wd)
...
sill.compare = combo.prep %>% 
  ...
  mutate(fin.range = ifelse(fin.max == -999 | 
       ...
       fin.sill = ifelse(fin.max == -999 | 
                              fin.min ==-999, NA, 
                            (fin.max+fin.min)/2)) %>% ungroup()

# Export the final sill elevation dataset!
setwd(int.wd)
...
# Map all those with low uncertainty
ggplot()+
  geom_sf(data=sill.sf %>% filter(!is.na(fin.sill)) %>% filter(fin.range/2 <=0.5),
          fill="black", color=NA)+
  theme_void()+
  geom_sf(data=mack.basin.large, color="grey70", size=0.5)+
 # geom_sf(data=station.locations.sill, color="black",size=2, shape=17)+
...
setwd(images.wd)
ggsave("lowuncert_sill.png", width = 4, height = 8.5, units="in")
...
# Plot uncertainty vs elevation
sill.sf %>% select(OBJECTID, fin.sill, fin.range) %>% 
  mutate(fin.uncert = fin.range/2) %>% as_tibble() %>% select(-geometry) %>% 
  filter(!is.na(fin.sill) & !is.na(fin.uncert)) %>% 
  ggplot()+
    geom_hex(aes(x=fin.sill, y=fin.uncert), binwidth = c(0.1, 0.1))+
    scale_fill_viridis_c(limits=c(0, 100), oob=squish,
  ...
  xlab("median functional connectivity\nelevation (m)")+
  ylab("uncertainty in functional\nconnectivity elevation threshold (m)")+
  labs(fill="number of\nlakes")+
  xlim(-0.25, 4)+ylim(-0.25, 4)+
  geom_abline(slope=1, intercept=0)+
  geom_text(aes(x=3, y=3.2, label="1:1"), angle=45)
...
ggsave("MarshHeyAll_versusPlot.pdf", width=4, height=4, units = "in")
...
ggsave("MarshHeyAll_noSillElev.pdf", width=3, height=3, units = "in")
# Plot summary table
